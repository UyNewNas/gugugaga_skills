...
async function geocodeAmap(address) {
  ...
  return { success: false, provider: 'amap', error: data.info || '地址解析失败' };
}
...
async function geocodeBaidu(address) {
  ...
  return { success: false, provider: 'baidu', error: data.message || '地址解析失败' };
}
...
async function geocodeTencent(address) {
  ...
  return { success: false, provider: 'tencent', error: data.message || '地址解析失败' };
}
...
export async function geocode(address, provider = null) {
  ...
  switch (targetProvider) {
    ...
    default:
      throw new Error(`未知的地图服务商: ${targetProvider}`);
  }
}
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('用法: node geocode.mjs <地址> [--provider <amap|baidu|tencent>]');
    console.error('\n示例:');
